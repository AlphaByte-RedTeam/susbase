            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow className="hover:bg-transparent border-b-2">
                  <TableHead className="w-[60px] text-center font-bold text-xs uppercase tracking-widest">Rank</TableHead>
                  <TableHead className="uppercase tracking-widest text-xs font-normal">Hunter</TableHead>
                  <TableHead className="text-right uppercase tracking-widest text-xs font-normal">Accepted Reports</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {leaderboard.length === 0 ? (
                  ...
                  leaderboard.map(([id, count], index) => {
                    ...
                    const displayName = isYou ? 'YOU' : `HUNTER-${id.substring(0, 10).toUpperCase()}`
                    ...
                    return (
                      ...
                      >
                        <TableCell className={cn("text-center font-mono font-bold text-lg", isYou && "text-primary")}>
                          {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `#${index + 1}`}
                        </TableCell>
                        <TableCell className="font-mono">
                          <div className="flex items-center gap-2">
                            <span className={cn("text-xs uppercase tracking-widest", isYou ? "font-bold text-primary" : "text-muted-foreground")}>
                              {displayName}
                            </span>
                            {isYou && <Badge variant="default" className="rounded-none text-[10px] px-1 h-4 bg-primary text-primary-foreground">Me</Badge>}
                          </div>
                        </TableCell>
                        <TableCell className={cn("text-right font-mono text-lg font-bold", isYou && "text-primary")}>
                          {count}
                        </TableCell>
                      </TableRow>
                    )
